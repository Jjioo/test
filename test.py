...
def computer_guess(x):
    ...
    while feedback != 'c'  :
            if(low !=high):
                ...
                random_guess=low
            feedback= input(f"is ({random_guess}) to High(H) or to Low(L) or Correct (C) --->").lower()
            if feedback =='h' :
                h=high
                high=random_guess -1
                while high> h :
...
#I am just a beginner and I tried to develop your mini project(GUESS THE NUMBER "USER")